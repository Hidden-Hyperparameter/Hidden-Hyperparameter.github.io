...
在给图片 $x$ 过了 Encoder $E$ 之后，我们将 Encoder 的每个 token $e_i$ 投影到和 $q_i$（来对齐 Codebook 的维度），然后在 codebook $C$ 里面找到最接近的 $p_i:=\operatorname{argmin}_{j}||c_j-q_i||$。这样我们就可以把这些 $q_i$ 量化成 $c_{p_i}$。我们希望解码时用 $D(\{c_{p_i}\}_i)$ 能得到原来的图片 $x$ 。
...
-  设 $x'=D(\{c_{p_i}\}_i)$，那么 $\mathcal L_{rec} = \mathcal l_2(x',x)+\mathcal L_{\text{LPIPS}}(D(x,x'))+\lambda_G\mathcal L_{G}(x')$。
...
- 其中 $\mathcal L_{distill} = ||E(x) - E_0(x)||^2$，其中 $E_0$ 是最开始的被 freeze 住的 VFM。 

### SimVQ Loss

最原始的 VQ 训练方法就是直接训练一些随机初始化的“Parameters” $c_j$，然后按照 $\mathcal L = \min_j(||q_i-c_j||^2)$ 对 $c$ 做梯度下降。这等价于在 argmin 位置加入 Loss $\mathcal L = ||q_i-c_{p_j}||^2$。

SimVQ 在此基础上做出了改进，极大地提升了 VQ Codebook 的利用效率：训练一个矩阵 $W$ 和一些 $c'_j$，然后令 $c_j = Wc'_j$，接着同样按照 $\mathcal L=\min_{j} (||q_i-c_j||^2)$ 梯度下降。其中矩阵 $W$ 是 $d\times d$ 的方阵，不改变 $c$ 的维度。
...
$$\mathcal L_{quant}=||sg(c_{p_j})-q_i||^2+\beta ||c_{p_j}-sg(q_i)||^2$$
