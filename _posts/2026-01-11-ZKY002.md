...
在给图片 $x$ 过了 Encoder $E$ 之后，我们将 Encoder 的每个 token $e_i$ 投影到和 $q_i$（来对齐 Codebook 的维度），然后在 codebook $C$ 里面找到最接近的 $p_i:=\operatorname{argmin}_{j}\mid \mid c_j-q_i\mid \mid $。这样我们就可以把这些 $q_i$ 量化成 $c_{p_i}$。我们希望解码时用 $D(\{c_{p_i}\}_i)$ 能得到原来的图片 $x$ 。
...
SimVQ 在此基础上做出了改进，极大地提升了 VQ Codebook 的利用效率：训练一个矩阵 $W$ 和一些 $c'_j$，然后令 $c_j = Wc'_j$，接着同样按照 $\mathcal L=\min_{j} (\mid \mid q_i-c_j\mid \mid ^2)$ 梯度下降。其中矩阵 $W$ 是 $d\times d$ 的方阵，不改变 $c$ 的维度。
